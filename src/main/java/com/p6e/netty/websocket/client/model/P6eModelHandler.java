...
public class P6eModelHandler implements ChannelInboundHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        ...
        if (!webSocketClientHandshaker.isHandshakeComplete()) {
            ...
            P6eWebSocketClient webSocket;
            if (id == null || (webSocket = p6eModelCache.get(id)) == null) return;
            ...
            if (frame instanceof BinaryWebSocketFrame) {
                if (id != null) p6eContextConverter.__onMessageBinary__(webSocket, bytes);
            } else if (frame instanceof TextWebSocketFrame) {
                if (id != null) p6eContextConverter.__onMessageText__(webSocket, new String(bytes));
            } else if (frame instanceof PongWebSocketFrame) {
                if (id != null) p6eContextConverter.__onMessagePong__(webSocket, bytes);
            } else if (frame instanceof PingWebSocketFrame) {
                if (id != null) p6eContextConverter.__onMessagePing__(webSocket, bytes);
            } else if (frame instanceof ContinuationWebSocketFrame) {
                if (id != null) p6eContextConverter.__onMessageContinuation__(webSocket, bytes);
            }
        }
    }
